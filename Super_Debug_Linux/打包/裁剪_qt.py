# -*- coding: UTF-8 -*-
"""
构建后裁剪：Super_Debug 只用 QtCore/Gui/Widgets/Network + OpenGL（投屏渲染）。
...
跨平台说明：
  - Windows 构建产物在 dist/Super_Debug/_internal（PySide6 模块为 .pyd，Qt 库为 qt6*.dll）。
  - macOS 构建产物在 dist/Super_Debug.app/Contents（模块为 .so，Qt 库为 libQt6*.dylib，
  ...
  - Linux 构建产物在 dist/Super_Debug/_internal（模块为 .so，Qt 库为 libQt6*.so，
...
KEEP_PLUGINS = {
    # 渲染平台：Win 只留 qwindows；Mac 只留 qcocoa；Linux 留 qxcb(X11) + qwayland(Wayland)
    'platforms': (['qwindows'] if IS_WIN else
                  (['qcocoa'] if IS_MAC else ['qxcb', 'qwayland', 'qminimal', 'qoffscreen'])),
...
def _trim_linux(internal):
    ...
    Linux 构建产物结构与 Windows 类似（dist/Super_Debug/_internal/），
    但 PySide6 模块为 .so，Qt 库为 libQt6*.so，插件为 libq*.so。
    闭包裁剪基于 bindepend 的 .so 依赖分析。
    """
    ps = os.path.join(internal, 'PySide6')
    if not os.path.isdir(ps):
        print('未找到', ps, '（先跑 精简打包exe.py）')
    ...
    # ---- 2) 计算保留 .so 的 libQt6*.so 传递依赖闭包 ----
    closure = set()
    if bindepend is not None:
        keep_so = [os.path.join(ps, m + MOD_EXT) for m in KEEP_MODS]
        stack = list(keep_so)
        while stack:
            f = stack.pop()
            try:
                imps = bindepend.get_imports(f, [ps, internal])
            except Exception as e:
                print('  跳过依赖解析', os.path.basename(f), e)
                continue
            for _name, path in imps:
                if path is None:
                    continue
                bn = os.path.basename(path).lower()
                if bn.startswith('libqt6') and (bn.endswith('.so') or '.so.' in bn):
                    if bn not in closure:
                        closure.add(bn)
                        stack.append(path)
        # 安全闸门：闭包必须包含 4 个核心库
        CORE = {'libqt6core.so', 'libqt6gui.so', 'libqt6widgets.so', 'libqt6network.so'}
        # Linux 上 .so 可能带版本号（libQt6Core.so.6），用前缀匹配
        def _in_closure(fn):
            low = fn.lower()
            return any(low.startswith(c) for c in CORE)
        if not any(_in_closure(c) for c in closure):
            print('ABORT: 依赖闭包异常(缺少核心库)，不删除任何 Qt6 库。closure=', sorted(closure))
            return
        print('libQt6*.so 依赖闭包:', sorted(closure))
        ...
        for f in os.listdir(ps):
            ...
            if low.startswith('libqt6') and (low.endswith('.so') or '.so.' in low):
                # 检查是否在闭包中（考虑版本号变体）
                in_closure = any(
                    low == c or low.startswith(c + '.')
                    for c in closure
                )
                if not in_closure:
                    action, full = _discard(ps, f, trash)
                    if action:
                        print(f'  [{action}] 闭包外 libQt6 库 {f}')
    else:
        print('  (跳过 Qt6 闭包裁剪：PyInstaller 不可用)')
...
def _make_trash(base):
    if os.environ.get('TRIM_MOVE'):
        ...
        # base 示例：dist/Super_Debug/_internal 或 dist/Super_Debug.app/Contents
...
def main():
    ...
    internal = os.path.join(here, 'dist', 'Super_Debug', '_internal')
    app_contents = os.path.join(here, 'dist', 'Super_Debug.app', 'Contents')
    if os.path.isdir(internal):
        ...
        print('未找到构建产物 dist/Super_Debug/_internal 或 dist/Super_Debug.app（先跑 精简打包exe.py）')
        return
