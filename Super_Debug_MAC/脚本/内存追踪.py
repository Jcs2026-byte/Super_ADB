...
4. 分阶段测量：
   S0 基线（仅最小 import）
   S1 导入 PySide6 核心
   S2 导入 Super_Debug_Win（此时重型子模块不应被加载 = 延迟 import 生效）
...
----
    ...
    python Super_Debug_Win/脚本/内存追踪.py
...
# 让 `import Super_Debug_Win` 能找到 Super_Debug_Win/Super_Debug_Win.py
# （该目录下没有 __init__.py，必须把它自身加入 sys.path）。
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_MAIN_DIR = os.path.abspath(os.path.join(_SCRIPT_DIR, ".."))  # -> Super_Debug_Win/
...
# --------------------------------------------------------------------------- #
HEAVY_MODULES = [
    ("设备性能监控", "DevicePerfMonitor"),
    ("Monkey压测窗口", "MonkeyRunnerWindow"),
    ("应用性能监控", "AppPerfMonitor"),
    ("安装解包对话框", "InstallZipDialog"),
    ("TCPDump对话框", "TcpdumpDialog"),
    ("关于对话框", "AboutDialog"),
    ("JSON工具对话框", "JsonToolDialog"),
    ("MD5对话框", "Md5Dialog"),
    ("时间戳对话框", "TimestampDialog"),
    ("无线调试对话框", "WirelessDebugDialog"),
    ("WiFi对话框", "WifiDialog"),
]
...
def main():
    ...
    import Super_Debug_Win  # noqa: F401
    s2 = measure("S2 +import Super_Debug_Win")
    ...
    if loaded_early:
        ...
        print("✓ 延迟 import 生效：import Super_Debug_Win 后 11 个重型子模块均未被加载")
    print("-" * 72)
    ...
    try:
        ...
        win = Super_Debug_Win.MainWindow()
