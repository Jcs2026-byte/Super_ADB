...
# 确保 Super_Debug_Win 根目录在 sys.path 中，支持 from 打包 import xxx
...
def _写入打包完成时间(base_dir, name='Super_Debug'):
...
def _重命名输出文件夹(base_dir, name='Super_Debug'):
    """打包完成后，把 dist/name 重命名为 dist/Super_Debug_<平台>（按平台命名）。

    Windows/Linux: dist/Super_Debug/ → dist/Super_Debug_Win/ (或 _Linux)
    macOS:         dist/Super_Debug.app → dist/Super_Debug_MAC.app
    目标已存在时先删除。
    """
    import shutil as _shutil
    _platform_suffix = _平台后缀()
    _target_name = f'Super_Debug_{_platform_suffix}'
    ...
    if _残留:
        raise RuntimeError(
            ...
            f'多半是上次打包出的 Super_Debug.exe 还在运行，文件被锁定。'
            f'请先关闭它（任务管理器结束 Super_Debug.exe）后重新打包。'
        )
    ...
    if _last_err is not None:
        raise RuntimeError(
            ...
            f'可能原因：1) Super_Debug.exe 正在运行，请先关闭再重试；'
...
def install(main):
    # 包式导入改造后，pathex 只需指向 Super_Debug_Win/ 根目录
    # 各子目录（对话框/页面/监控/工具/项目UI）均含 __init__.py 成为正规包，
    # PyInstaller 通过根包路径自动发现所有子包模块。
    here = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.dirname(here)
    # 项目UI目录也加入 paths，因为 Super_Debug.py 中是裸导入 import png_rc
    ...
    # Super_Debug_Win/外部扩展 才能和源码目录结构保持一致。
    ...
    if sys.platform == 'darwin':
        ...
        icon = os.path.join(base_dir, 'adb.icns') if os.path.exists(os.path.join(base_dir, 'adb.icns')) else os.path.join(base_dir, '资源', 'Super_Debug.png')
        ...
        # Windows: 生成 .exe
        icon = os.path.join(base_dir, '资源', 'Super_Debug.png')
        cmd = f'pyinstaller --clean -w -i "{icon}" -n {name} --distpath "{base_dir}/打包/dist" --workpath "{base_dir}/打包/build" {hidden} {hooks} {runtime_hooks} {excludes} {res_arg} {data_arg} {path_args} "{main}"'
    os.system(cmd)
    print('配置文件生成成功')
    ...
    # 打包完成后，输出文件夹按平台命名（Super_Debug_Win / _MAC / _Linux）
...
def install1(s):
    ...
    os.system(install)
    print('打包完成')
    # 打包完成后写入时间配置（从参数解析 -n 应用名，默认 Super_Debug）
    _here = os.path.dirname(os.path.abspath(__file__))
    _base = os.path.dirname(_here)
    _m = re.search(r'-n\s+(\S+)', s)
    _name = _m.group(1) if _m else 'Super_Debug'
    _写入打包完成时间(_base, _name)
    _重命名输出文件夹(_base, _name)


if __name__ == '__main__':
    install(os.path.join("项目启动入口", "Super_Debug_主入口.py"))
