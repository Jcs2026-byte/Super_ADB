#!/bin/bash
# ==============================================================================
# Super_Debug — macOS 一键打包 + 签名 + ZIP 分发脚本（自包含版）
# ==============================================================================
# 用法（在项目根目录任意位置执行均可）：
...
#   唯一的打包配置 打包/Super_Debug_mac.spec（由 spec 负责把 libusb 的 dylib
#   随包内置、挂运行时钩子、声明全部隐藏依赖），最终产出可直接分发的 ZIP。
#   若你的环境已经存在可用解释器（见下方优先级），不会重复下载任何东西。
#
# 构建用 Python 解析优先级：
#   1) 环境变量 Super_Debug_PYTHON 指定的路径（显式覆盖）
...
#
# 产物：
#   打包/dist/Super_Debug_MAC.app     — 深度签名后的应用包
#   打包/dist/Super_Debug_mac.zip     — 可分发的 ZIP 压缩包
#
# 用户拿到 ZIP 后：
#   1. 解压得到 Super_Debug.app
#   2. 拖入 /Applications（或直接双击运行）
#   3. 首次启动：右键 Super_Debug → 打开（绕过 Gatekeeper）
#   4. 之后可正常双击打开
# ==============================================================================
...
APP_PATH="$DIST_DIR/Super_Debug_MAC.app"
ZIP_PATH="$DIST_DIR/Super_Debug_mac.zip"
...
PYTHON=""
# 1) 环境变量显式指定（最高优先）
if [[ -n "$Super_Debug_PYTHON" && -x "$Super_Debug_PYTHON" ]]; then
    PYTHON="$Super_Debug_PYTHON"
fi
...
if [[ -z "$PYTHON" ]]; then
    warn "未找到系统 python3，将在 打包/.build_venv 创建虚拟环境并安装依赖（首次较慢）。"
...
# 具体 dylib 路径由 Super_Debug_mac.spec 按本机架构动态定位并随包）
ensure_dep "libusb"     "libusb"

# ── 2. 清理旧产物（移到 /tmp 回收目录，可恢复，且避免环境批量删除限制）────────
info "清理旧构建产物..."
TRASH="/tmp/Super_Debug_trash_$(date +%s)"
mkdir -p "$TRASH"
moved=0
for p in "$DIST_DIR/Super_Debug_MAC.app" "$DIST_DIR/Super_Debug_MAC" "$DIST_DIR/Super_Debug.app" "$DIST_DIR/Super_Debug" "$ZIP_PATH" "$PROJECT_ROOT/打包/build"; do
    if [[ -e "$p" ]]; then
...
info "开始 PyInstaller 打包（直接执行 打包/Super_Debug_mac.spec，这可能需要 1-3 分钟）..."
cd "$PROJECT_ROOT"
"$PYTHON" -m PyInstaller --noconfirm \
    --distpath "$DIST_DIR" --workpath "$PROJECT_ROOT/打包/build" \
    打包/Super_Debug_mac.spec
...
# 打包信息.json（下载地址/版本）写入应用内 配置/，供关于页读取。
info "写入打包信息（配置/打包信息.json）..."
"$PYTHON" -c "import sys; sys.path.insert(0, r'$PROJECT_ROOT/打包'); import 精简打包exe; 精简打包exe._写入打包完成时间(r'$PROJECT_ROOT', 'Super_Debug_MAC')" 2>&1 | tail -2 || true
...
# 校验：ZIP 必须含 配置/打包信息.json 与 libusb（避免 ditto 丢中文目录的坑）
"$PYTHON" - "$ZIP_PATH" <<'PY'
import sys, zipfile
z = zipfile.ZipFile(sys.argv[1])
names = z.namelist()
has_info = any('配置/打包信息.json' in n for n in names)
has_libusb = any('libusb-1.0.dylib' in n for n in names)
print('  zip 含 配置/打包信息.json:', has_info)
...
PY
if [[ $? -ne 0 ]]; then
    error "ZIP 缺少关键文件（配置/打包信息.json 或 libusb），请检查 make_zip.py"
fi
...
echo "  1. 用户解压 Super_Debug_mac.zip，得到 Super_Debug_MAC.app"
echo "  2. 将 Super_Debug_MAC.app 拖入 /Applications（或直接双击运行）"
echo "  3. 首次启动：右键 Super_Debug_MAC → 打开"
...
echo "  用户在终端执行："
echo "    xattr -d com.apple.quarantine /Applications/Super_Debug_MAC.app"
echo ""
echo -e "${YELLOW}── 校验 ZIP 完整性（可选）──${NC}"
echo "  用户可执行："
echo "    shasum -a 256 Super_Debug_mac.zip"
