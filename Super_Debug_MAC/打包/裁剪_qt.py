...
def main():
    ...
    if IS_MAC:
        ...
        # 优先识别 Super_Debug_MAC.app（build_mac_zip.sh 走 spec 的产物名），
        # 回退 Super_Debug.app（精简打包exe.py 旧命名）以兼容。
        app_contents = None
        for nm in ('Super_Debug_MAC.app', 'Super_Debug.app'):
        ...
        if app_contents:
            ...
            print('未找到构建产物 dist/Super_Debug_MAC.app/Contents 或'
                  ' dist/Super_Debug.app/Contents（先跑构建）')
        return
