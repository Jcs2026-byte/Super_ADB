...
├── Super_Debug_linux.spec    # Linux 专用 PyInstaller 配置
├── requirements_linux.txt  # Linux 运行依赖清单
├── 精简打包exe.py           # 跨平台 Python 打包脚本（Windows/macOS/Linux）
├── 裁剪_qt.py               # 构建后 Qt 库裁剪工具（跨平台）
├── Super_Debug.spec          # Windows spec 文件
├── Super_Debug_mac.spec      # macOS spec 文件
...
# 自动安装依赖后打包
bash 打包/build_linux.sh --install-deps

# 清理旧构建后打包
bash 打包/build_linux.sh --clean

# 打包并生成 tar.gz 压缩包
bash 打包/build_linux.sh --archive

# 完整流程：安装依赖 + 清理 + 打包 + 压缩
bash 打包/build_linux.sh --install-deps --clean --archive
```
...
```bash
# 1. 安装依赖
pip install -r 打包/requirements_linux.txt

# 2. 使用 spec 文件打包
pyinstaller --clean --noconfirm 打包/Super_Debug_linux.spec

# 3. 修复执行权限
chmod +x 打包/dist/Super_Debug/Super_Debug
find 打包/dist/Super_Debug -name "adb" -o -name "scrcpy" | xargs chmod +x

# 4. 运行
./打包/dist/Super_Debug/Super_Debug
```

## 跨平台 Python 打包脚本

`精简打包exe.py` 支持 Windows/macOS/Linux 三平台：
...
```bash
sed -i 's/\r$//' 打包/build_linux.sh
# 或
dos2unix 打包/build_linux.sh
```
