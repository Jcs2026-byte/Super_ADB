...
MAIN = os.path.join(os.path.dirname(__file__), "..")  # -> Super_Debug_Win
sys.path.insert(0, os.path.abspath(MAIN))
sys.path.insert(0, os.path.abspath(os.path.join(MAIN, "工具")))
sys.path.insert(0, os.path.abspath(os.path.join(MAIN, "项目UI")))

# 用独立临时配置文件，绝不碰真实 Super_ADB配置.json
_TMP = tempfile.mkdtemp(prefix="superadb_geo_")
TEMP_CONFIG = os.path.join(_TMP, "Super_ADB配置.json")
...
# 让 Super_Debug_Win 模块读到我们自己的临时配置
...
def _monkeypatch_config(mod):
    ...
    mod.加载json配置 = load_json_config
    mod.保存json配置 = save_json_config


def _load_main():
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "Super_ADB_Win_geo", os.path.join(MAIN, "项目启动入口", "Super_Debug_主入口.py"))
