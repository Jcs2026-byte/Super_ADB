# -*- coding: utf-8 -*-
"""验证 _config_path 重构：配置统一放 配置/ 子目录 + adb_shell_config.json → Super_ADB配置.json 迁移"""
import os
import sys
import json

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # Super_Debug_Win
...
# ---- 1. 路径解析：主配置名 ----
p1 = _config_path('配置/Super_ADB配置.json')
check("主配置路径含 配置/ 子目录",
      p1 and p1.endswith(os.path.join('配置', 'Super_ADB配置.json')),
      f"resolved={p1}")

# ---- 2. 路径解析：裸文件名（secondary configs）----
p2 = _config_path('wifi_debug_history.json')
check("裸名 wifi_debug_history.json 也落在 配置/ 下",
      p2 and '配置' in p2 and p2.endswith(os.path.join('配置', 'wifi_debug_history.json')),
      f"resolved={p2}")

p3 = _config_path('wifi_paired_devices.json')
check("裸名 wifi_paired_devices.json 也落在 配置/ 下",
      p3 and '配置' in p3 and p3.endswith(os.path.join('配置', 'wifi_paired_devices.json')),
      f"resolved={p3}")

# ---- 3. 迁移测试：adb_shell_config.json → Super_ADB配置.json ----
old_main = os.path.join(BASE, '配置', 'adb_shell_config.json')
new_main = os.path.join(BASE, '配置', 'Super_ADB配置.json')
...
scatter_target = os.path.join(BASE, '配置', scatter_name)
...
check("散落在 base 下的文件迁移到 配置/", not os.path.isfile(scatter_base) and os.path.isfile(scatter_target),
      f"resolved={resolved}")
