...
========================================
iOS 系统日志跟随：
    子进程跑 pymobiledevice3 syslog live → 后台线程逐行入队 → 主线程 QTimer 缓冲批量追加，
...
class cIos日志跟随对话框(对话框基类):
    ...
    def f开始(self):
        if self._b运行中 or not self._bios工具.f可用():
            return
        _b命令 = self._bios工具.f构造命令(self._budid, 'syslog', 'live')
        try:
            ...
            from 工具.Ios调试模块.pymd3桥接 import f安装指引
        ...
        self._b状态标签.setText('正在跟随 pymobiledevice3 syslog live …'
                                '（iOS 17+ 的隧道由 pymobiledevice3 按需自动建立）')
