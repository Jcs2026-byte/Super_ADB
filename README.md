...
- 主题样式丰富
- **最新版本：`2026.09.14.02`**（GitHub / Gitee 均已同步）
...
**方法 1：右键打开（最快）**
在 Finder 里找到 `Super_ADB_MAC.app`，**按住 Control 点它（或右键）→ 打开**，弹窗里再点一次 "打开"。之后这个 app 就被记住，双击能直接开。

**方法 2：系统设置里放行**
刚才那个弹窗点 "完成"，然后：
系统设置 → 隐私与安全性 → 拉到最下面，会看到 "已阻止使用 Super_ADB_MAC…" → 点右边的**仍要打开**。

**方法 3：命令行去隔离属性（CI 自用最干脆，一劳永逸）**
打开 "终端"，执行（把路径换成你实际放 app 的位置，一般拖 app 进终端会自动填路径）：
