...
mkdir -p "$TRASH"
moved=0
for p in "$DIST_DIR/Super_ADB_MAC.app" "$DIST_DIR/Super_ADB_MAC" "$DIST_DIR/Super_ADB.app" "$DIST_DIR/Super_ADB" "$ZIP_PATH" "$PROJECT_ROOT/打包/build"; do
    if [[ -e "$p" ]]; then
...
info "开始 PyInstaller 打包（直接执行 打包/Super_ADB_mac.spec，这可能需要 1-3 分钟）..."
cd "$PROJECT_ROOT"
"$PYTHON" -m PyInstaller --noconfirm \
    --distpath "$DIST_DIR" --workpath "$PROJECT_ROOT/打包/build" \
    打包/Super_ADB_mac.spec
...
"$PYTHON" -c "import sys; sys.path.insert(0, r'$PROJECT_ROOT/打包'); import 精简打包exe; 精简打包exe._写入打包完成时间(r'$PROJECT_ROOT', 'Super_ADB_MAC')" 2>&1 | tail -2 || true
