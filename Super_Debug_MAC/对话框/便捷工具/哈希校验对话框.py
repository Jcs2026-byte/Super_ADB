...
class 哈希校验对话框(对话框基类):
    ...
    def _install_context_menu(self):
        try:
            ...
            # 故源码版用 Super_Debug.ico，冻结版直接复用 exe 自带图标（"exe",0）最稳妥。
            icon_path = ''
            if getattr(sys, 'frozen', False):
                ...
                icon_candidates.append(os.path.join(_file_dir, '../../../Super_Debug_Linux', 'ui', 'Super_Debug.ico'))
                icon_candidates.append(os.path.join(_file_dir, '../../../Super_Debug_Linux', '资源', 'Super_Debug.ico'))
                icon_candidates.append(os.path.join(_file_dir, '../../../Super_Debug_Linux', 'Super_Debug.ico'))
                # 冻结版目录兜底（开发态若与打包目录混用）
                icon_candidates.append(os.path.join(os.path.dirname(exe), 'Super_Debug.ico'))
