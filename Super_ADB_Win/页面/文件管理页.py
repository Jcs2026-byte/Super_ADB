...
from 工具.android调试模块.ADB工具 import (AdbFileManager, 格式化设备标签,
                                          加载json配置, 保存json配置)
...
# 首次启动默认值（源自 Super_ADB配置.json 的 col_ratios）
...
class 文件管理页(QWidget):
    def __init__(self, parent=None):
        ...
        # 定时刷新（仅刷新已展开的目录），默认 3 秒一次
        self._auto_refresh_ms = 3000
    ...
    def inject_widgets(self, *, tree: QTreeView, device_combo: QComboBox,
                       ...
                       btn_device_mgr: QPushButton = None):
        ...
        # 设备管理器开关（默认关闭）：打开才加载/刷新文件列表
        self.btn_device_mgr = btn_device_mgr
        if self.btn_device_mgr is not None:
            self.btn_device_mgr.setCheckable(True)
            self.btn_device_mgr.setChecked(False)
            self.btn_device_mgr.setText('打开设备管理器')
            self.btn_device_mgr.clicked.connect(self._on_toggle_device_mgr)
            self._device_mgr_on = False
    ...
    def _build_ui(self):
        ...
        # 设备管理器开关（默认关闭）
        self.btn_device_mgr = QPushButton('打开设备管理器')
        self.btn_device_mgr.setCheckable(True)
        self.btn_device_mgr.setChecked(False)
        self.btn_device_mgr.setToolTip('打开后加载并定时刷新文件列表；关闭状态下不执行获取文件')
        self.btn_device_mgr.clicked.connect(self._on_toggle_device_mgr)
        bar.addWidget(self.btn_device_mgr)
    ...
    def _on_device(self):
        ...
        if self._device_mgr_on:
            ...
            self._status('设备管理器已关闭，点击「打开设备管理器」后加载文件')
    ...
    def _toggle_root(self):
        ...
        if self._current_serial:
            if self._device_mgr_on:
                ...
                self._status('请先打开设备管理器')

    def _build_root(self):
        if not self._device_mgr_on:
            self._clear_tree()
            return
    ...
    # ------------------------------------------------------------------
    # 设备管理器开关 & 定时刷新（仅刷新已展开目录）
    # ------------------------------------------------------------------
    def _on_toggle_device_mgr(self):
        self._device_mgr_on = bool(self.btn_device_mgr.isChecked())
        if self._device_mgr_on:
            ...
            if self._current_serial:
                ...
                self._status('请先选择设备')
            self._auto_refresh_timer.start(self._auto_refresh_ms)
        else:
            self.btn_device_mgr.setText('打开设备管理器')
            self._auto_refresh_timer.stop()
            self._clear_tree()
    ...
    def _populate(self, item, entries):
        ...
        只更新变化的行、删除消失的、追加新增的，避免整树清空重建导致闪烁。
        ...
        new_list = dirs + files
        new_paths = {e['path'] for e in new_list}

        # 1) 记录当前展开的子目录路径（重建后恢复）
        expanded_sub = set()
        for r in range(item.rowCount()):
            child = item.child(r, 0)
            entry = child.data(Qt.UserRole) if child else None
            p = entry.get('path') if entry else None
            if p and self.tree.isExpanded(child.index()):
                expanded_sub.add(p)

        # 2) 删除消失的行 + 占位伪子项（无 UserRole 的空行）
        ...
        for i, e in enumerate(new_list):
            cur = item.child(i, 0)
            cur_entry = cur.data(Qt.UserRole) if cur else None
            if cur_entry and cur_entry.get('path') == e['path']:
                # 位置正确：就地更新文本/数据，其余列重建
                self._update_row(item, i, e)
                continue
            # 在剩余行中查找该 path 的节点 → 移动复用到位置 i
            found = False
            for r in range(i, item.rowCount()):
                c2 = item.child(r, 0)
                en2 = c2.data(Qt.UserRole) if c2 else None
                if en2 and en2.get('path') == e['path']:
                    row_items = item.takeRow(r)
                    item.insertRow(i, row_items)
                    self._update_row(item, i, e)
                    found = True
                    break
            if not found:
                ...
                item.insertRow(i, [child, QStandardItem(sz), QStandardItem(e['perm']), QStandardItem(e['mtime'])])

        # 4) 恢复被刷新目录自身及其已展开子目录的展开状态
