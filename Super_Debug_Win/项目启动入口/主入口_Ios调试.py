...
from 工具.Ios调试模块.pymd3桥接 import (
    cIos设备操作, f安装指引,
)
...
class cIos调试Mixin:
    ...
    def f初始化Ios调试模块(self):
        ...
        self.btnIosProxy.clicked.connect(self.f打开代理设置)
        # 系统操作
    ...
    def f刷新Ios设备(self):
        ...
        if not self.ios.f可用():
            self.设置状态('未找到 pymobiledevice3，请先安装（可点「环境自检」看指引）', ok=False)
    ...
    def _fIos设备列表完成时(self, _b设备列表):
        ...
        # 直接读 currentData 记录原选择，避免空下拉时误报「请先选择设备」
        _b原选择 = str(self.iosDeviceCombo.currentData() or '')
    ...
    def _fIos异步执行(self, _b函数, _b完成槽, *_b参数):
        ...
        from 项目启动入口.Super_Debug_主入口 import 命令工作器
    ...
    def f打开代理设置(self):
        """打开 iOS 全局 HTTP 代理设置（含 SSL 证书安装）。"""
        if not self._fIos需要设备():
            return
        from 对话框.Ios调试模块.iOS代理设置对话框 import cIos代理设置对话框
        _b窗口 = self._bios代理窗口
        if _b窗口 is not None:
            try:
                if _b窗口.isVisible():
                ...
                self._bios代理窗口 = None
        _b窗口 = cIos代理设置对话框(self.ios, self.f当前IosUdID(), self)
        _b窗口.destroyed.connect(
            lambda _b对象=None, _b自身=self: _b自身._fIos清空窗口引用('_bios代理窗口', _b对象))
        self._bios代理窗口 = _b窗口
        _b窗口.show()

    def f管理隧道(self):
        """查看常驻隧道；无隧道时询问是否启动 tunneld 常驻服务（可选）。"""
        if not self.ios.f可用():
            self.fIos日志(f'错误: {f安装指引()}')
            return
        try:
        ...
        _b询问 = QMessageBox.question(
            self, '启动隧道',
            '当前没有常驻隧道（tunneld 未运行）。\n'
            '说明：iOS 17+ 的隧道 pymobiledevice3 会在需要时自动建立'
            '（userspace 方式，免 root），无常驻隧道通常也能正常调试。\n\n'
            '是否启动 tunneld 常驻服务？启动后可加速重复连接'
            '（监听 127.0.0.1:49151）。',
    ...
    # ------------------------------------------------------------------
    def f输出原始信息(self):
        """详情：输出 pymobiledevice3 version + lockdown info 原始 JSON 到输出区。"""
        if not self._fIos需要设备():
            return
        def _f取原始():
            ...
            return (f'$ pymobiledevice3 version\n{_b版本}\n\n'
                    f'$ pymobiledevice3 lockdown info\n'
                    + json.dumps(_b信息, ensure_ascii=False, indent=2))
    ...
    def _fIos采样一帧(self):
        ...
        _b行 = ['采样结果（字段以 pymobiledevice3 实际输出为准）:']
