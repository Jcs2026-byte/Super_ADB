...
本 spec 是 macOS 构建的唯一配置来源，build_mac_zip.sh 在装好依赖后会直接执行：
    pyinstaller --distpath 打包/dist --workpath 打包/build 打包/Super_ADB_mac.spec
...
# 找不到模块而报错，故条件化）。
_hiddenimports = [
    'segno', 'segno.helpers', 'zeroconf', 'ifaddr', 'pyzbar', '工具.android调试工具.收藏下拉框',
    'png_rc', '项目UI.png_rc',
    # ★ 自研ADB新增依赖（与 Super_ADB_Win/打包/Super_ADB.spec 同步，缺一不可）：
