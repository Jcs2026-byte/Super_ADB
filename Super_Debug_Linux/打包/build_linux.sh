#!/usr/bin/env bash
# ==============================================================================
# Super_Debug Linux 一键打包脚本
# ==============================================================================
# 用法：
#   bash 打包/build_linux.sh              # 常规打包
#   bash 打包/build_linux.sh --install-deps  # 先安装依赖再打包
#   bash 打包/build_linux.sh --clean      # 清理旧构建后重新打包
#   bash 打包/build_linux.sh --archive    # 打包完成后生成 tar.gz 压缩包
#   bash 打包/build_linux.sh --help       # 显示帮助
#
# 产物：
#   打包/dist/Super_Debug/          可执行文件目录（onedir 模式）
#   打包/dist/Super_Debug/Super_Debug 主程序
# ==============================================================================
...
用法:
  bash 打包/build_linux.sh [选项]

选项:
  --install-deps   打包前自动安装 Python 依赖（pip install -r requirements_linux.txt）
  --clean          打包前清理旧的 build/ 和 dist/ 目录
  --archive        打包完成后生成 Super_Debug_linux_x86_64.tar.gz 压缩包
  --skip-exec-fix  跳过外部工具执行权限修复步骤
  --help           显示此帮助信息

示例:
  bash 打包/build_linux.sh                          # 常规打包
  bash 打包/build_linux.sh --clean --archive        # 清理后打包并生成压缩包
  bash 打包/build_linux.sh --install-deps --clean   # 安装依赖、清理、打包

产物位置:
  打包/dist/Super_Debug/Super_Debug    主可执行文件
  打包/dist/Super_Debug/              完整运行目录（含依赖和外部工具）
EOF
}
...
DIST_DIR="$SCRIPT_DIR/dist/Super_Debug"
BUILD_DIR="$SCRIPT_DIR/build"
SPEC_FILE="$SCRIPT_DIR/Super_Debug_linux.spec"
...
ADB_DIR="$PROJECT_ROOT/外部扩展/adb"
SCRCPY_DIR="$PROJECT_ROOT/外部扩展/scrcpy"
...
if [[ ! -f "$DIST_DIR/Super_Debug" ]]; then
    log_error "打包失败：未找到产物 $DIST_DIR/Super_Debug"
    exit 1
fi
log_ok "打包成功: $DIST_DIR/Super_Debug"
...
if [[ "$SKIP_EXEC_FIX" != true ]]; then
    ...
    # 也修复主程序执行权限
    if [[ -f "$DIST_DIR/Super_Debug" && ! -x "$DIST_DIR/Super_Debug" ]]; then
        chmod +x "$DIST_DIR/Super_Debug"
        ((FIXED_COUNT++)) || true
    fi
...
if command -v du &>/dev/null; then
    ...
    EXE_SIZE="$(du -h "$DIST_DIR/Super_Debug" 2>/dev/null | cut -f1)"
else
    DIST_SIZE="未知"
    EXE_SIZE="未知"
fi
...
if [[ "$MAKE_ARCHIVE" == true ]]; then
    ARCHIVE_NAME="Super_Debug_linux_$(uname -m)_$(date +%Y%m%d_%H%M%S).tar.gz"
    ARCHIVE_PATH="$SCRIPT_DIR/dist/$ARCHIVE_NAME"
    log_info "生成压缩包: $ARCHIVE_NAME"
    tar -czf "$ARCHIVE_PATH" -C "$SCRIPT_DIR/dist" "Super_Debug"
...
echo -e "  主程序:     ${CYAN}$DIST_DIR/Super_Debug${NC}"
