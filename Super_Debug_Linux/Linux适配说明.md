...
| # | 模块 | 文件 | 适配前状态 | 适配后状态 |
|---|------|------|-----------|-----------|
| 1 | 配置文件路径 | `工具/ADB工具.py` | Linux 冻结版走 exe 旁，AppImage 只读 | ✅ 遵循 XDG 规范走 `~/.config/Super_ADB/` |
| 2 | 外部工具权限 | `工具/ADB工具.py` | adb/scrcpy 从 Windows 拷贝后无执行权限 | ✅ 自动检测并 `chmod +x` |
| 3 | 本机 WiFi 密码 | `工具/WiFi工具.py` | Linux 报"操作系统不支持" | ✅ 通过 NetworkManager (nmcli) 读取 |
| 4 | WiFi 密码恢复 | `工具/WiFi密码破解.py` | Linux 直接调 netsh 失败 | ✅ 复用 WiFi工具.py 跨平台实现 |
| 5 | 打包脚本 | `打包/精简打包exe.py` | 仅 Windows/macOS 分支 | ✅ 新增 Linux 分支 |
| 6 | Qt 库裁剪 | `打包/裁剪_qt.py` | Linux 走 Windows 闭包逻辑会崩 | ✅ 新增 `_trim_linux()` 分支 |
...
**文件**：`工具/ADB工具.py` → 新增 `_ensure_executable()`

**问题**：项目中的 `外部扩展/adb/platform-tools-latest-linux/` 和 `外部扩展/scrcpy/scrcpy-linux-x86_64-v4.1/` 下的二进制文件从 Windows 环境拷贝后可能丢失执行权限，导致 `Permission denied`。
...
**文件**：`工具/WiFi密码破解.py` → `cmd_recover()`

**问题**：原 `cmd_recover` 直接调用 `netsh wlan`，非 Windows 平台会 `FileNotFoundError`。

**修复**：在函数开头添加平台分支，非 Windows 时动态加载 `WiFi工具.py` 并调用其 `collect_all()` 跨平台实现，输出格式与 Windows 版一致。
...
- 图标用 `资源/Super_ADB.png`
- 输出目录与 Windows 一致：`打包/dist/Super_ADB/`
...
| 模块 | 文件 | 跨平台机制 |
|------|------|-----------|
| ADB 路径探测 | `工具/ADB工具.py` | `find_bundled_adb_path()` 已有 `platform-tools-latest-linux` 分支 |
| scrcpy 探测 | `工具/ADB工具.py` | `find_scrcpy_dir()` 已有 `scrcpy-linux-` 前缀 |
| scrcpy 启动 | `工具/ADB工具.py` | `is_win` 分支控制 exe 名和 render-driver |
| logcat 终端 | `工具/ADB工具.py` | 已有 `x-terminal-emulator` Linux 分支 |
| 剪贴板输入 | `项目启动入口/Super_ADB_主入口.py` | `is_win` 分支，非 Windows 走 `QGuiApplication.clipboard()` |
| 窗口发光重画 | `项目UI/弹窗样式.py` | 所有 `ctypes.windll` 调用均在 `if sys.platform == 'win32':` 守卫内 |
| 字体选择 | `项目UI/界面样式.py` | 已有 `linux → Noto Sans CJK SC` 分支 |
| 环境配置/PATH | `对话框/环境配置对话框.py` | `add_to_user_path()` 已有 Linux `~/.bashrc` 分支 |
| 哈希右键菜单 | `对话框/哈希校验对话框.py` | `winreg` 条件导入，非 Windows 自动隐藏按钮 |
| 外部二进制 | `外部扩展/adb/` + `外部扩展/scrcpy/` | 已预置 Linux x86_64 版本 |
...
产物位于 `打包/dist/Super_ADB/`，可执行文件为 `Super_ADB`（无扩展名）。

### 5.3 打包后验证

```bash
# 检查外部工具执行权限
ls -la 打包/dist/Super_ADB/_internal/外部扩展/adb/platform-tools-latest-linux/platform-tools/adb
ls -la 打包/dist/Super_ADB/_internal/外部扩展/scrcpy/scrcpy-linux-x86_64-v4.1/scrcpy

# 运行
./打包/dist/Super_ADB/Super_ADB
```
