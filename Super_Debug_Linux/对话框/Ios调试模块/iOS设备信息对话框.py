...
=============================================
读取 lockdown info，中文映射展示常用字段，下方可切原始 JSON。
...
# pymobiledevice3 lockdown info 键 → 中文名（多个候选键依次探测）
_b字段映射 = [
    ('设备名称', ('DeviceName',)),
    ('系统版本', ('ProductVersion',)),
    ('构建版本', ('BuildVersion',)),
...
class cIos设备信息对话框(对话框基类):
    ...
    def _f构建界面(self):
        ...
        self._b状态标签 = QLabel('正在读取 lockdown info …')
        _b外层.addWidget(self._b状态标签)

    # ------------------------------------------------------------------
    def f加载信息(self):
        self._b状态标签.setText('正在读取 lockdown info …')
