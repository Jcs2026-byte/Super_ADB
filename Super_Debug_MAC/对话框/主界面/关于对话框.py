...
- 提供 ``apply_theme(theme_id=None)``：
  - 默认 ``theme_id`` → 从父窗口 ``_current_theme`` 读，缺省回落到
    ``界面样式.DEFAULT_THEME``
  - 主窗口切换主题后通过 ``Super_Debug_Win._传播主题到弹窗``
    把新主题同步到已打开的弹窗
...
from 项目UI import png_rc  # noqa: F401   # 注册 :/Super_Debug.png 与 :/qrcode.jpg 资源
...
VERSION = 'v2026.08.07'
GITHUB_REPO_URL = 'https://github.com/Jcs2026-byte/Super_Debug.git'
    ...
    try:
        if getattr(sys, 'frozen', False):
            ...
            if sys.platform == 'darwin':
                ...
                _base = os.path.join(_base, '../../../Super_Debug_Linux', 'Resources')
        else:
...
class 关于对话框(QDialog, 无边框缩放Mixin):
    ...
    def __init__(self, parent=None):
        ...
        self.resize(440, 620)
        self.setWindowTitle('关于 Super_Debug')
        self.setWindowIcon(QIcon(':/Super_Debug.png'))
        ...
        self.title_lbl = QLabel('关于 Super_Debug')
