...
class 二维码连接页(QWidget):
    ...
    def _start_waiting(self):
        ...
        if not self._service_name or not self._code:
            return
        try:
            from 工具.Android调试模块.自研adb.mdns发现 import (
    ...
    def _候选设备ip(self):
        ...
        try:
            from 工具.Android调试模块.自研adb.mdns发现 import get_connect_services
    ...
    def _stop_waiting(self):
        ...
        if self._mdns_bridge is not None:
            try:
                from 工具.Android调试模块.自研adb.mdns发现 import unregister_pairing_listener
    ...
    def _on_discovered(self, name, ip, port):
        ...
        try:
            from 工具.Android调试模块.自研adb.mdns发现 import ensure_running
            ensure_running()
        except Exception:
            pass
    ...
    def _on_qr_pair_done(self, ok, msg):
        ...
        if ok:
            ...
            if self._pair_dialog is not None:
                ...
                try:
                    from 工具.Android调试模块.自研adb.mdns发现 import get_connect_port
    ...
    def _on_connect_success(self, ip, port):
        ...
        try:
            from 工具.Android调试模块.自研adb.mdns发现 import stop as mdns_stop
            mdns_stop()
        except Exception:
            pass
    ...
    def _popup_qr(self):
        ...
        dlg.setWindowIcon(QIcon(":/Super_Debug.png"))
        add_green_glow(dlg)
