...
"""
Super_Debug Linux 打包配置
========================
使用相对路径，基于本 spec 文件所在目录（打包/）的上一级（项目根）。
生成 onedir 模式的可执行文件（dist/Super_Debug/Super_Debug）。

用法：
    cd 项目根目录
    pyinstaller 打包/Super_Debug_linux.spec
...
# 图标：Linux 用 .png
_ICON = os.path.join(_PROJECT_ROOT, '资源', 'Super_Debug.png')
...
a = Analysis(
    ...
    hiddenimports=['segno', 'segno.helpers', 'zeroconf', 'ifaddr', 'pyzbar', '工具.收藏下拉框',
              ...
              '项目UI.对话框基类',
              '工具.自研adb.mdns发现', 'usb', 'usb.core', 'usb.util', 'usb.backend.libusb1'],
