...
for version in ['Super_Debug_MAC', 'Super_Debug_Win', 'Super_Debug_Linux']:
    project_root = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), version)
    print('=== ' + version + ' ===')
    total_all = 0
    for root, dirs, files in os.walk(project_root):
        if '.git' in root or '__pycache__' in root or '脚本' in root:
            continue
        for fn in files:
