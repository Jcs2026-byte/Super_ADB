...
- 内置 ADB 路径探测：与 ``ADB工具.查找scrcpy目录`` 同样的「base / parent / cwd」三级回退，
  覆盖源码模式 ``Super_Debug_Win/外部扩展/...`` 与冻结模式 ``_internal/外部扩展/...`` 两种布局
"""
...
def detect_socket_adb():
    ...
    """
    try:
        from 工具.Android调试模块.ADB协议客户端 import Adb协议客户端, 检查server运行, 启动adb服务器
        if not 检查server运行():
            启动adb服务器()
            if not 检查server运行():
                return None, None
...
class 环境配置对话框(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.auto_connect_chk = QCheckBox('启动时自动扫描局域网 android ios 设备并连接')
    ...
    def _probe_sync(self, mode: str) -> dict:
        ...
        if mode == 'system':
            ...
            from 工具.Android调试模块.ADB工具 import 查找系统adb路径
    ...
    def _重启adb进程同步(self, 模式: str):
        ...
        if 模式 in ('system', 'socket'):
            from 工具.Android调试模块.ADB工具 import 查找系统adb路径, 查找内置adb路径
            adb_path = None
            if 模式 == 'system':
                adb_path = 查找系统adb路径()
            if not adb_path:
